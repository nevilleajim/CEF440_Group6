...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    if (!_isInitialized) {
      _networkService = Provider.of<NetworkService>(context, listen: false);
      _currentCarrier = _networkService.currentCarrier;
      _isInitialized = true;
    }
  }
  ...
  void _startNetworkMonitoring() {
    _networkService = Provider.of<NetworkService>(context, listen: false);
    _networkService.startMonitoring();
    _networkService.addListener(_updateNetworkData);
    _networkService.metricsStream.listen((metrics) {
      if (mounted) {
        setState(() {
          _currentMetrics = metrics;
          _currentCarrier = metrics.carrier;
        });
      }
    });
  }
  ...
  void _updateNetworkData() {
    if (mounted && _networkService.currentMetrics != null) {
      setState(() {
        _currentMetrics = _networkService.currentMetrics;
        _currentCarrier = _networkService.currentCarrier;
      });
    }
  }
  ...
  void dispose() {
    ...
    _networkService.removeListener(_updateNetworkData);
    super.dispose();
  }
  ...
  Widget _buildNetworkStatusCard() {
    String quality = _networkService.getNetworkQualityStatus(_currentMetrics);
    Color qualityColor = _networkService.getQualityColor(quality);
              ...
              children: [
                Expanded(
                  child: _buildStatusItem(
                    'Provider',
                    _currentCarrier, // Use the stored carrier value
                ...
                Expanded(
                  child: _buildStatusItem(
                    'Type',
                    _currentMetrics?.networkType ?? 'Mobile',
    ...
    final stats = [
      {
        ...
        'quality':
            _networkService.getMetricQuality('download', _currentMetrics?.downloadSpeed),
      },
      {
        'title': 'Upload',
        ...
        'quality': _networkService.getMetricQuality('upload', _currentMetrics?.uploadSpeed),
      },
      {
        'title': 'Latency',
        ...
        'quality': _networkService.getMetricQuality('latency', _currentMetrics?.latency),
      },
      {
        'title': 'Jitter',
        ...
        'quality': _networkService.getMetricQuality('jitter', _currentMetrics?.jitter),
      },
    ];
